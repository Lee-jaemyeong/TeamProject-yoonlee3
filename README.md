...
<details>
  ...
  • 문제 상황: 카카오 로그아웃 시, 세션 쿠키가 정상적으로 삭제되지 않아 사용자가 로그아웃해도 자동으로 로그인 상태가 유지되는 현상이 발생 → 보안 취약점 존재
  • 원인 분석: 카카오 로그아웃 API 호출 또는 로그아웃 URL 리다이렉션이 누락 → 카카오 측 세션 및 인증 토큰이 미해제
  • 해결 방법: Spring Security 설정에 카카오 로그아웃 URL을 명시하여 로그아웃 요청 시 해당URL로 리다이렉트되도록 구현 → 카카오 세션을 종료시키고, 클라이언트 쿠키도 정상적으로 삭제되도록 처리
